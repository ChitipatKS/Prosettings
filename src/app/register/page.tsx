...
function RegisterForm() {
  ...
  return (
    <div className="w-full max-w-md bg-[#12121A]/80 border border-zinc-800 p-8 rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.3)] backdrop-blur-md">
      ...
      {successMsg && (
        <div className="bg-green-500/10 border border-green-500/20 text-green-400 text-xs font-mono p-4 rounded-xl mb-6">
          {successMsg}
        </div>
      )}
      ...
      <div className="mt-8 border-t border-zinc-800/60 pt-6 text-center text-xs text-zinc-400 font-mono">
        ...
        <Link href={`/login${searchParams.toString() ? '?' + searchParams.toString() : ''}`} className="text-accent hover:underline">
          Sign in
        </Link>
      </div>
    </div>
